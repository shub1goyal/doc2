...
- When creating tables, use the number of columns appropriate for the requested layout (e.g. 3-column format | Metric | Value | Pages | for basic metric lists, or a multi-column format with custom headers if comparing years, categories, or adding explanations as explicitly asked in the prompt)
...
function getTaskSchemaAndFormatter(taskName) {
    ...
    // If the task name contains "variance", it outputs a JSON matching VarianceTask1Schema and formats it as a markdown table
...
function parseTasksFromPrompt(promptText) {
    const tasks = [];
    const parts = promptText.split(/(?=\*\*Task \d+[A-Z]?:)/g);
